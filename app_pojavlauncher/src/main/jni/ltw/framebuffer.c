...
GLenum glCheckFramebufferStatus( 	GLenum target) {
    ...
    /* ltw-lite: a separate stencil attachment was merged into DEPTH_STENCIL_ATTACHMENT,
     * so report COMPLETE instead of INCOMPLETE_ATTACHMENT. */
    if(framebuffer_status == GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT) {
        framebuffer_t *framebuffer = get_framebuffer(target);
        if(framebuffer && framebuffer->stencil_rb) return GL_FRAMEBUFFER_COMPLETE;
    }
...
void glFramebufferTexture2D( 	GLenum target,
    ...
    if(!current_context) return;
    framebuffer_t *framebuffer = get_framebuffer(target);
    /* ltw-lite: depth texture attachment -> combined DEPTH_STENCIL_ATTACHMENT */
    if(attachment == GL_DEPTH_ATTACHMENT) {
        GLuint obj = (framebuffer && framebuffer->stencil_rb) ? framebuffer->stencil_rb : texture;
        if(texture == 0) { rebind_framebuffer(target, framebuffer, attachment); return; }
        framebuffer->stencil_rb = obj;
        es3_functions.glFramebufferTexture2D(target, GL_DEPTH_STENCIL_ATTACHMENT, textarget, obj, level);
        return;
    }
...
void glFramebufferRenderbuffer( 	GLenum target,
    ...
    /* ltw-lite: merge separate depth/stencil into one DEPTH_STENCIL_ATTACHMENT.
     * GLES3 has no GL_STENCIL_INDEX8 / GL_STENCIL_ATTACHMENT. This is what keeps
     * Create's enableStencil() from throwing GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT. */
    if(attachment == GL_STENCIL_ATTACHMENT) {
        if(framebuffer) framebuffer->stencil_rb = renderbuffer;
        es3_functions.glFramebufferRenderbuffer(target, GL_DEPTH_STENCIL_ATTACHMENT, renderbuffertarget, renderbuffer);
        return;
    }
    if(attachment == GL_DEPTH_ATTACHMENT) {
        GLuint rb = (framebuffer && framebuffer->stencil_rb) ? framebuffer->stencil_rb : renderbuffer;
        es3_functions.glFramebufferRenderbuffer(target, GL_DEPTH_STENCIL_ATTACHMENT, renderbuffertarget, rb);
        return;
    }
